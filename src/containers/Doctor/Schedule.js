...
class Schedule extends Component {
	...
	render() {
		...
		const { loaded, offset, currentDate, appointmentId } = this.state;
			...
			<Fragment>
	          ...
	          <Grid item container xs={12} style={{ width: maxWidth, padding: small ? "1rem" : "2rem 0 1rem" }}>
	            <Grid item container xs={12} direction="column" style={{ margin: "1rem 0 0.75rem" }}>
	              <span style={{ fontSize: "2rem", color: 'white', fontWeight: 500 }}>{empty(appointment) ? "My Schedule" : `${moment.utc(appointment.appointmentdate).format('MM/DD/YY')} Appointment`}</span>
          ...
          {empty(appointment) && (<Fragment>
	            ...
	            <Grid xs={12} container item>
								...
								<Grid container item xs={2} direction="column" style={{ borderRight: "1px solid #ddd", position: "relative" }}>
									...
									{replaceTime(appointments.filter(item => moment.utc(item.appointmentdate).format('MM-DD-YYYY') === addWeekday(currentDate, offset).format('MM-DD-YYYY'))).map((row, i) => (
										<Fragment key={i}>
											{row.pid && (<Link to={`/doctor/schedule/${row.id}`} style={{width: "100%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", background: i % 2 === 0 ? "#f2f2f2" : "white", height: "2rem", padding: "0.5rem 0"}}>
	          ...
	          <Grid xs={12} item container direction="column" alignItems="center" style={{ height: "100%", marginBottom: "1rem"}}>
		          <Grid item container xs={12} direction="column" style={{ width: maxWidth, padding: "0.5rem 0"}}>
					...
					{!empty(appointment) && (<Grid xs={12} item container direction="column" alignItems="center" style={{  minHeight: "calc(100vh - 4rem)"}}>
...
export default connect(mapStateToProps, { getAppointments })(withRouter(withToast(Schedule)));
